...
namespace ai
{
  namespace PL
  {
                                    ...
                                    std::vector<Symbol *> &pure_symbols)
    {
      ...
      unsigned int i = 0;
      for(sym_it = unassigned_symbols.begin(); sym_it != unassigned_symbols.end(); sym_it++, i++)
        {
      ...
      /* All of this to the end of the function is to remove the
       * pure_symbols from the unassigned_symbols
       * needs to remove pure_symbols from unassigned_symbols */
#if 0
      std::cerr << std::endl;
      std::cerr << "pure_symbols: ";
      for(i = 0; i < pure_symbols.size(); i++)
        {
          std::cerr << pure_symbols[i]->GetName() << " ";
        }
      std::cerr << std::endl;
      std::cerr << "unassigned_symbols: ";
      for(i = 0; i < unassigned_symbols.size(); i++)
        {
          std::cerr << unassigned_symbols[i]->GetName() << " ";
        }
      std::cerr << std::endl;
#endif
      for(i = 0; i < remove_indexes.size(); i++)
        {
          unassigned_symbols[remove_indexes[i]] = 0;
        }
#if 0
      std::cerr << "unassigned_symbols: ";
      for(i = 0; i < unassigned_symbols.size(); i++)
        {
          if(unassigned_symbols[i])
            {
              std::cerr << unassigned_symbols[i]->GetName() << " ";
            }
          else
...
#endif
      unsigned int j;
      for(i = 0; i < unassigned_symbols.size() - remove_indexes.size(); i++)
        {
          if(!unassigned_symbols[i])
            {
              for(j = unassigned_symbols.size(); j > 0; j--)
                {
                  if(unassigned_symbols[j-1])
                    {
                      break;
                    }
                }
              if(j > 0 && unassigned_symbols[j-1])
                {
                  unassigned_symbols[i] = unassigned_symbols[j-1];
                  unassigned_symbols[j-1] = 0;
                }
            }
        }
#if 0
      std::cerr << "unassigned_symbols post: ";
      for(i = 0; i < unassigned_symbols.size(); i++)
        {
          if(unassigned_symbols[i])
            {
    ...
    Symbol::State KnowledgeBase::DPLL_Entails(KnowledgeBase &alpha)
    {
      ...
      if(alpha.sentences.size() != 1)
        {
          std::cerr << "Designed for questions with exactly one sentence. " 
                    << "You have " << alpha.sentences.size() << " sentences." << std::endl;
          return Symbol::UNKNOWN;
        }
                                          ...
                                          std::vector<Symbol *>   &unassigned_symbols)
    {
...
#if 0
                unsigned int j;
                std::cerr << "UnitClause: " << *(unknown_clauses[i]) << std::endl;
                std::cerr << "UnitSymbol: " << unit_symbol->GetName() << std::endl;

                std::cerr << "Unassigned: ";
                for(j = 0; j < unassigned_symbols.size(); j++)
                  { std::cerr << unassigned_symbols[j]->GetName() << " "; }
                std::cerr << std::endl;

                std::cerr << "Assigned: ";
                for(j = 0; j < assigned_symbols.size(); j++)
                  { std::cerr << assigned_symbols[j]->GetName() << " "; }
                std::cerr << std::endl;
#endif
                break;
              }
      ...
      std::cout << std::endl;
#endif
      /* expect pure_symbols to be removed from unassigned_symbols */
